...
export default function CustomChatSidebar() {
  ...
  return <div className="relative flex h-full min-h-0 flex-col bg-background">
    ...
    <div className="flex items-center justify-between border-b bg-background/95 px-4 py-3 backdrop-blur"><div className="flex min-w-0 items-center gap-2.5"><div className="rounded-lg border border-primary/15 bg-primary/10 p-1.5"><Sparkles className="h-4 w-4 text-primary" /></div><div className="min-w-0"><p className="text-sm font-semibold">Vio Assistant</p><p className="truncate text-[11px] text-muted-foreground">Grounded answers from your workspace</p></div></div><div className="flex items-center gap-1"><Badge variant="outline" className="hidden h-6 text-[10px] sm:inline-flex">Built-in AI</Badge><Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-muted-foreground" onClick={() => void clear()} title="Clear conversation"><Trash2 className="h-4 w-4" /></Button></div></div>
