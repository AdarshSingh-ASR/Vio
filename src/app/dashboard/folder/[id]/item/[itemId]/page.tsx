...
const fetchFolderItems = async (getAuthenticatedFetch: () => any, folderId: string): Promise<DashboardItem[]> => {
  try {
    const authenticatedFetch = getAuthenticatedFetch();
    const res = await authenticatedFetch(`/api/dashboard/items?folderId=${folderId}`);
    if (!res.ok) return [];
...
const FolderItemViewerPage = () => {
  const router = useRouter();
  const params = useParams();
  const folderId = typeof params.id === 'string' ? params.id : Array.isArray(params.id) ? params.id[0] : '';
  ...
  // Use TanStack Query for folder items
  const { data: items = [], isLoading: itemsLoading } = useQuery({
    queryKey: ['folder-items', folderId],
    queryFn: () => fetchFolderItems(getAuthenticatedFetch, folderId),
    enabled: !!folderId,
  });
  ...
  useEffect(() => {
    ...
    const idx = items.findIndex(item => (item.$id || item.id) === itemId);
    setCurrentIndex(idx >= 0 ? idx : 0);
  }, [itemId, items, folderId]);
  ...
  // Navigation functions for folder context
  const navigateToItem = (index: number) => {
    if (index >= 0 && index < items.length) {
      const targetItem = items[index];
      const targetItemId = targetItem.$id || targetItem.id;
      if (targetItemId) {
        router.push(`/dashboard/folder/${folderId}/item/${targetItemId}`);
      }
    }
  };

  if (itemsLoading) return <div className="flex justify-center items-center min-h-screen">Loading...</div>;
  if (!items.length) return <div className="flex justify-center items-center min-h-screen">No items found in this folder.</div>;
  const currentItem = items[currentIndex];
  if (!currentItem) return <div className="flex justify-center items-center min-h-screen">Item not found in folder.</div>;
      ...
      {showUpNextModal && !upNextCollapsed && (
        <div className="fixed inset-4 top-20 md:bottom-4 md:right-4 md:top-auto md:left-auto z-[100] w-auto md:w-[320px] md:max-w-[calc(100vw-2rem)] bg-white dark:bg-card border-2 border-primary rounded-xl shadow-2xl flex flex-col"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <span className="font-bold text-lg text-foreground">Up Next in Folder</span>
            <button
            ...
            {items.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  onClick={() => { navigateToItem(idx); setShowUpNextModal(false); setUpNextCollapsed(false); }}
      ...
      {showUpNextModal && upNextCollapsed && !chatSheetOpen && (
        ...
        >
          <div className="text-white">
            <span className="font-bold">Next in Folder:</span> {items[currentIndex + 1]?.displayName || items[currentIndex + 1]?.name || '—'}
          </div>
      ...
      >
          ...
          <div className="w-full flex flex-col items-start max-w-4xl">
            ...
            <div className="w-full flex items-center justify-between mb-4">
              <button
                ...
                onClick={() => router.push(`/dashboard/folder/${folderId}`)}
        ...
        {!showUpNextModal && (
          <div className="md:hidden w-full px-3 mt-6 mb-6">
            <div className="font-bold text-base mb-3 text-foreground">Up Next in Folder</div>
